...
io.on('connection', function(socket) {
  ...
  socket.on('ledUpdate', function(data) {
    // data = { countdown: ..., targetState: 'GREEN' or 'RED' }
    console.log('Received ledUpdate:', data);
  
    // 发送指令给 Arduino
    if (arduino && arduino.isOpen) {
      arduino.write(data.targetState + '\n');
    }
    // 同时广播给其他客户端（如果你想同步显示）
    io.emit('instruction', data.targetState === 'GREEN' ? 'Green Light ON' : 'Red Light ON');
  });
});
...
function updateLED() {
  ...
  for (let id in pointers) {
    if (Date.now() - pointers[id].lastSeen > 3000) {
      ...
      if (pointers[id].x < canvasWidth / 2) {
        leftZoneCount++;
      } else {
        rightZoneCount++;
      }
  ...
  console.log('Green zone:', leftZoneCount, 'Red zone:', rightZoneCount);
  ...
  if (leftZoneCount > rightZoneCount) {
    instruction = 'Green Light ON';
    arduinoCommand = 'GREEN';
  } else if (rightZoneCount > leftZoneCount) {
    instruction = 'Red Light ON';
    arduinoCommand = 'RED';
  } else {
    instruction = 'Equal - No change';
    arduinoCommand = 'OFF';
  }
  // Send instruction to all clients
  io.emit('instruction', instruction);
  // 发送指令给Arduino
  if (arduino && arduino.isOpen) {
